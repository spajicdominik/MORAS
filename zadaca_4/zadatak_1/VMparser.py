class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if l[2].isdigit() and (int(l[2])>=0):
                    return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n)
                else:
                    self._flag = False
                    Parser._error("Parser", n, "push argument must be integer");
                    return ""
            else:
                self._flag = False
            ...
            if len(l) == 3:
                if l[2].isdigit() and (int(l[2])>=0):
                    ...
                    self._flag = False
                    Parser._error("Parser", n, "pop argument must be integer");
                    return ""
            else:
                self._flag = False
                Parser._error("Parser", n, "Undefined command");
            ...
            if l[0] == "label" and len(l) == 2:
                ...
                if l[1] in self._labels:
                    ...
                    self._flag = False
                    Parser._error("Parser", n, "invalid label!");
                    return ""
