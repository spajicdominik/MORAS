...
def _parse_macros(self,line,p,o):
    ...
    if macro == "MV":
        ...
        if len(k) != 2:
            ...
            x = k[0]
            y = k[1]
            return ["@" + str(x),"D=M", "@" + str(y), "M=D"]
        
    elif macro == "SWP":
        k = line[5:len(line)-1].split(",")
        if len(k) != 2:
            self._flag = False
            self._line = o
            self._errm = "Invalid macro!"
            ...
            x = k[0]
            y = k[1]
            return ["@" + str(x) , "D=M", "@" + "t", "M=D", "@" + str(y) ,"D=M" ,"@" + str(x), "M=D" ,"@" +"t","D=M", "@" + str(y) ,"M=D"]
    elif macro == "SUM":
        k = line[5:len(line)-1].split(",")
        if len(k) != 3:
            self._flag = False
            ...
            y = k[1]
            z = k[2]
            return ["@"+ str(x) ,"D=M" ,"@" + str(y) ,"D=M+D" ,"@" + str(z) ,"M=D"]
